...
function[studyPowerVals, com] = pop_calculatePower(EEG, windowSize, whoAmI, plotMe, export_csv, filename)
...
if nargin < 2
    ...
    a = inputdlg({'Window size in data segments?', ...
        ...
        'Calculate Power',[1 100; 1 100; 1 100; 1 100; 1 100],...
        cell({'2','allPSDs.mat','y','y','ROIpower.csv'}), options);
...
try
    studyPowerVals = evalin('base','studyPowerVals'); 
...
% f = linspace(0,sRate/2,sRate/2+1);  % frequency bins in increments of 1
% delete me
rf = floor(f); % rounded frequency bins

powerAllChans = zeros((EEG.nbchan+1),length(f)); % blank vector to hold PSDs for all channels

for i = 1:(NFFT/2+1)
% for i = 1:(sRate/2+1) delete me
...
try
    ...
    [x1,x2,c] = size(allPSDs); % x2 = num of cols of allPSDs; c = count number of layers
    [y1,y2]   = size(powerAllChans); % y1 = num of rows ; y2 = num of cols of powerAllChans
    % Pad zeros
    if x2 > y2    % pad y with zeros
        temp = powerAllChans;
        powerAllChans = zeros(y1,x2);
        powerAllChans(:,1:y2) = temp;

    elseif y2 > x2
        temp    = allPSDs;
        allPSDs = zeros(x1,y2,c);
        allPSDs(:,1:x2,:) = temp;
